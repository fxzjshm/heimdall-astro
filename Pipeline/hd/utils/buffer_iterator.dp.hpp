...
{
    std::string identifier = kernel.get_buffer_identifier<T>(expr.m_buffer, expr.m_address_space);
    size_t offset_count = kernel.get_and_increase_offset_counter();
    std::string offset_name = identifier + std::string("_offset") + std::to_string(offset_count);
    kernel.add_set_arg(offset_name, static_cast<uint_>(expr.m_index));
    return kernel << identifier << '[' << offset_name << "+(" << expr.m_expr << ")]";
    // return kernel << identifier << '[' << expr.m_index << "+(" << expr.m_expr << ")]";
}
/*
template<class T, class IndexExpr>
inline meta_kernel& operator<<(meta_kernel &kernel,
                               const buffer_iterator_index_expr<T, boost::compute::detail::meta_kernel_variable<IndexExpr>> &expr)
{
    if(expr.m_index == 0){
        return kernel <<
...
inline meta_kernel &operator<<(meta_kernel &kernel, const buffer_iterator<T> &iter) {
    std::string identifier = kernel.get_buffer_identifier<T>(iter.get_buffer());
    size_t offset_count = kernel.get_and_increase_offset_counter();
    std::string offset_name = identifier + std::string("_offset") + std::to_string(offset_count);
    kernel.add_set_arg(offset_name, static_cast<uint_>(iter.get_index()));
    return kernel << '(' << identifier << '+' << offset_name << ')';
    // return kernel << '(' << identifier << '+' << iter.get_index() << ')';
}
