...
// modification: correctly set output iterator offset in `write_scanned_output_kernel`
...
template<class T, class BinaryOperator, class OutputIterator>
class write_scanned_output_kernel : public meta_kernel
{
public:
    write_scanned_output_kernel(BinaryOperator op, OutputIterator output)
        : meta_kernel("write_scanned_output")
    {
        ...
        *this << decl<T>("tmp") << " = " << output[var<uint_>("gid")] << ";\n";

        // write output
        *this <<
            output[var<uint_>("gid")] << " = " <<
                op(var<T>("block_sums[block_id]"), var<T>("tmp")) << ";\n";
                                ...
                                command_queue &queue)
{
    ...
    if(block_count > 1){
        write_scanned_output_kernel<input_type, BinaryOperator, OutputIterator>
            write_output_kernel(op, result);
