...
// modification: store buffer offset as arguments instead of meta_kernel_literals
...
public:
    ...
    explicit meta_kernel(const std::string &name)
        : m_name(name), m_offset_counter(0)
    {
    }
    ...
    meta_kernel(const meta_kernel &other)
    {
        ...
        m_offset_counter = other.m_offset_counter;
    ...
    size_t get_and_increase_offset_counter()
    {
        size_t ret = m_offset_counter;
        m_offset_counter++;
        return ret;
    }
...
private:
    ...
    volatile size_t m_offset_counter;
};
